...
def extract_artist_dim(input_csv, geo_dim_csv, output_csv):
    # load geo_dim to map FK
    geo_map = {}
    with open(geo_dim_csv, 'r') as f:
        reader = csv.reader(f)
        next(reader)
        for row in reader:
    ...
    for row in data:
        geo_key = (row[birthplace_idx], row[region_idx], row[country_idx])
        geo_fk = geo_map.get(geo_key)
...
def extract_time_dim(input_csv, output_csv):
    ...
    with open(output_csv, 'w', newline='') as f:
        ...
        for (dow, y, m, s), tid in time_records.items():
            writer.writerow([tid, dow, y, m, s])

def extract_track_dim(input_csv, time_dim_csv, output_csv):
    time_map = {}
    with open(time_dim_csv, 'r') as f:
        reader = csv.reader(f)
        next(reader)
        for row in reader:
            time_map[(row[2], row[3])] = row[0]  # year, month → time_id
    ...
    for row in data:
        ...
        key = (row[id_idx], row[title_idx], time_fk)
    ...
    with open(output_csv, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['ID_track', 'ID_track_original', 'song_category', 'ID_time_FK'])
        for (orig, title, tfk), tid in track_records.items():
            writer.writerow([tid, orig, title, tfk])

def extract_track_artist_bridge(input_csv, track_dim_csv, artist_dim_csv, output_csv):
    track_map = {}
    with open(track_dim_csv, 'r') as f:
        reader = csv.reader(f)
        next(reader)
        for row in reader:
    ...
    artist_map = {}
    with open(artist_dim_csv, 'r') as f:
        reader = csv.reader(f)
        next(reader)
        for row in reader:
        ...
        if track_fk and artist_fk:
            ...
            if row[feat_idx]:
                for feat in row[feat_idx].split(','):
                    records.append((track_fk, feat.strip(), 'feature', 0.2))
...
input_file = r"/Users/huynhphuongchi/Desktop/Unipi/DSS/Module 2/LDS Data 2025-2026/artists.csv"
...
extract_geo_dim(input_csv, geo_dim_csv)
extract_artist_dim(input_csv, geo_dim_csv, artist_dim_csv)
extract_time_dim(input_csv, time_dim_csv)
extract_track_dim(input_csv, time_dim_csv, track_dim_csv)
extract_track_artist_bridge(input_csv, track_dim_csv, artist_dim_csv, bridge_csv)
extract_fact_table(input_csv, track_dim_csv, fact_csv)